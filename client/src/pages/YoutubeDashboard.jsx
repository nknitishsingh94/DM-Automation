    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%', padding: '20px', fontFamily: 'Inter, system-ui, sans-serif', animation: 'fadeIn 0.5s ease-out', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid var(--border-subtle)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: '900', color: 'var(--text-main)', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
              YouTube Studio
            ...
            <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: '500' }}>Manage, schedule, and optimize your channel</p>
          </div>
        </div>
        ...
        {channelStats.map((stat, i) => (
          <div key={i} style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '20px', border: '1px solid var(--border-subtle)', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
              <h2 style={{ fontSize: '1.8rem', fontWeight: '800', margin: 0, color: 'var(--text-main)' }}>{stat.value}</h2>
      ...
      <div style={{ flex: 1 }}>
              ...
              {videoLibrary.map(video => (
                <div key={video.id} style={{ background: 'var(--bg-card)', borderRadius: '16px', overflow: 'hidden', border: '1px solid var(--border-subtle)', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'transform 0.2s', cursor: 'pointer' }} onMouseOver={e => e.currentTarget.style.transform = 'translateY(-4px)'} onMouseOut={e => e.currentTarget.style.transform = 'none'}>
                  ...
                  <div style={{ padding: '16px' }}>
                    <h3 style={{ margin: '0 0 8px 0', fontSize: '1.05rem', color: 'var(--text-main)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: '1.4' }}>{video.title}</h3>
        ...
        {activeTab === 'thumbnails' && (
          <div 
            ...
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px', background: isDraggingThumb ? '#eff6ff' : 'var(--sidebar-bg)', borderRadius: '16px', border: `2px dashed ${isDraggingThumb ? '#3b82f6' : 'var(--border-subtle)'}`, padding: '32px', cursor: !generatedThumb ? 'pointer' : 'default', transition: 'all 0.2s ease' }}
          >
            ...
            {!generatedThumb ? (
              <div onClick={(e) => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '400px', pointerEvents: isDraggingThumb ? 'none' : 'auto' }}>
                ...
                <h2 style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--text-main)', margin: '0 0 8px 0', textAlign: 'center' }}>AI Thumbnail Generator</h2>
                <p style={{ color: 'var(--text-muted)', textAlign: 'center', marginBottom: '24px' }}>Let our AI analyze your video title and generate a high-converting thumbnail, or drag and drop your own image here.</p>
                <input 
                  type="text" 
                  value={aiPrompt}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, animation: 'fadeIn 0.2s ease-out' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '24px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            ...
            <div style={{ padding: '24px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: 'var(--bg-card)', zIndex: 10 }}>
              ...
              <button onClick={() => setShowScheduleModal(false)} style={{ background: 'var(--bg-dark)', border: 'none', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>×</button>
            </div>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div 
                ...
                style={{ border: scheduleData.mediaUrl ? '2px solid #10b981' : '2px dashed #cbd5e1', borderRadius: '16px', padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--sidebar-bg)', cursor: 'pointer' }}
                onMouseOver={e => !scheduleData.mediaUrl && (e.currentTarget.style.borderColor = '#ff0000')} 
                onMouseOut={e => !scheduleData.mediaUrl && (e.currentTarget.style.borderColor = 'var(--border-subtle)')}
              >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <label style={{ fontSize: '0.95rem', fontWeight: '700', color: 'var(--text-main)' }}>Title (required)</label>
                ...
                <div>
                  <label style={{ fontSize: '0.95rem', fontWeight: '700', color: 'var(--text-main)', marginBottom: '8px', display: 'block' }}>Description</label>
                  <textarea 
                ...
                <div>
                  <label style={{ fontSize: '0.95rem', fontWeight: '700', color: 'var(--text-main)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {/* Scheduling Section */}
              <div style={{ background: 'var(--bg-dark)', padding: '20px', borderRadius: '12px' }}>
                <h4 style={{ margin: '0 0 16px 0', fontSize: '1rem', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px' }}><Calendar size={18} /> Visibility & Schedule</h4>
                <div style={{ display: 'flex', gap: '16px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-muted)', marginBottom: '4px', display: 'block' }}>Date</label>
                    <input 
                      type="date" 
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-muted)', marginBottom: '4px', display: 'block' }}>Time</label>
                    <input 
                      type="time" 
            ...
            {isUploading && (
              <div style={{ padding: '0 24px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-muted)' }}>
      ...
      {showAIModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, animation: 'fadeIn 0.2s ease-out' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '20px', width: '400px', padding: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            ...
            <h3 style={{ margin: '0 0 8px 0', fontSize: '1.2rem', color: 'var(--text-main)' }}>AI Content Optimizer</h3>
