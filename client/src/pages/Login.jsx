...
import { Bot, LogIn, Mail, Lock, Info, Facebook, Eye, EyeOff } from 'lucide-react';
...
export default function Login() {
  ...
  const handleGoogleResponse = async (response) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { login(data.user, data.token); navigate('/connections'); }
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="login-input-wrap">
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <Lock size={17} className="login-input-icon" style={{ top: '50%', transform: 'translateY(-50%)' }} />
                  <input
                    className="login-input"
                    ...
                    placeholder="••••••••"
                    style={{ paddingRight: '45px', fontSize: '1.15rem', height: '48px' }}
                  />
                  <button
                    type="button"
