  ...
  return (
    <div style={{ padding: '0 40px 40px' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '15px' }}>Invite friends to smart10X and earn amazing rewards for every successful referral.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '32px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
          ...
          {/* How it works */}
          <div className="stat-card" style={{ padding: '32px' }}>
            <h3 style={{ fontSize: '17px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '20px' }}>How it works</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <h4 style={{ fontSize: '14px', fontWeight: '700', color: 'var(--text-main)', marginBottom: '2px' }}>{s.title}</h4>
        ...
        <div className="stat-card" style={{ padding: '32px', height: 'fit-content' }}>
          <h3 style={{ fontSize: '17px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '6px' }}>Reward Tiers</h3>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '24px' }}>Unlock bigger rewards as you refer more friends.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {rewards.map((r, i) => (
              <div key={i} style={{ padding: '16px 20px', borderRadius: '16px', border: `1px solid ${r.locked ? 'var(--bg-dark)' : r.color + '40'}`, background: r.locked ? '#fafafa' : r.color + '08', display: 'flex', alignItems: 'center', gap: '14px' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: r.locked ? 'var(--bg-dark)' : r.color + '15', color: r.locked ? 'var(--text-muted)' : r.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {r.icon}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: '600', marginBottom: '2px' }}>{r.tier}</div>
