...
export const publishInstagramContent = async (userId, { type, mediaUrl, caption = '', carouselItems = [], containerId = null }, workspaceId = null) => {
  try {
    ...
    if (!finalCreationId) {
      ...
      if (type === 'carousel' && carouselItems.length > 0) {
        const childPromises = carouselItems.map(async (itemUrl) => {
          ...
          const childRes = await axios.post(`https://graph.facebook.com/v19.0/${igId}/media`, null, { params: childParams, timeout: 60000 });
        ...
        const containerRes = await axios.post(containerUrl, params, { timeout: 60000 });
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        publishRes = await axios.post(publishUrl, null, { timeout: 60000 });
