...
router.get('/linkedin', verifyToken, (req, res) => {
  ...
  let scope = 'openid%20profile%20w_member_social';
  if (isBusiness) {
...
router.get('/linkedin/callback', async (req, res) => {
  ...
  try {
    ...
    let personUrn = '';
    try {
      // First try OIDC userinfo endpoint (needs 'profile' scope)
      const userinfoRes = await axios.get('https://api.linkedin.com/v2/userinfo', {
        headers: { Authorization: `Bearer ${accessToken}` }
      });
      if (userinfoRes.data) {
        if (userinfoRes.data.sub) personUrn = `urn:li:person:${userinfoRes.data.sub}`;
        if (userinfoRes.data.name) profileName = userinfoRes.data.name;
        else if (userinfoRes.data.given_name) profileName = `${userinfoRes.data.given_name} ${userinfoRes.data.family_name || ''}`.trim();
      }
    } catch (e) {
      console.warn("Could not fetch LinkedIn profile from /v2/userinfo:", e.message);
    }

    if (profileName === 'LinkedIn Member') {
      try {
        ...
        if (profileRes.data) {
          if (profileRes.data.id && !personUrn) {
        ...
        console.warn("Could not fetch LinkedIn profile from /v2/me:", profileErr.response?.data || profileErr.message);
      }
    }
